...
async function loadPrayerTimes(lat, lon) {
  try {
    ...
    for (let key in prayerElements) {
      prayerElements[key].textContent = formatTime(cleanTime(timings[key]));
    }
...
function updateNextPrayer(timings) {
  ...
  for (let p of prayers) {
    const time = cleanTime(p[1]).split(":");
    const d = new Date();
    d.setHours(+time[0]);
    d.setMinutes(+time[1]);
    d.setSeconds(0);
  ...
  if (!next) {
    const time = cleanTime(prayers[0][1]).split(":");
    const d = new Date();
